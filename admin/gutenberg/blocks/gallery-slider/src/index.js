...
registerBlockType('hupa/gallery-slideshow-block', {
    ...
    save({attributes}) {
        ...
        if (layout === 'gallery') {
            return (
                <div data-focus-active={showFocus} data-lightbox-single={lightboxSingle} data-focus={thumbFocus} data-slider={selectedSlider} className="thumbnail-slider">
                    <section id={`main-carousel-${sliderId}`}
                        ...
                        <div className="splide__track">
                            <div style={{height: sliderHeight ? sliderHeight + 'px' : '100%'}} className="splide__list">
                                {images.map((image) => (
                                    <a target={image.tab ? '_blank' : '_self'} href={clickAction === 'lightbox' ? image.url : image.link !== '' ? image.link : ''} className={`splide__slide ${clickAction === 'lightbox' || clickAction === 'individuell' && image.link !== '' ? 'cursor-pointer' : 'pe-none'}`} key={image.id}
                                    >
                                        <img style={{objectPosition: objectPosition, objectFit: objectFit, height: sliderHeight ? sliderHeight + 'px' : '100%'}}
        ...
        return (
            <div data-focus-active={showFocus} data-lightbox-single={lightboxSingle} data-focus={thumbFocus} data-slider={selectedSlider} className="theme-slider">
                <div id={`slider-${sliderId}`} className={`slideshow splide ${clickAction === 'lightbox' ? 'with-lightbox' : ''}`}>
                    <div className="splide__track">
                        <div className={`splide__list ${images.length < 3 ? 'justify-content-center' : ''}`}>
                            {images.map((image) => (
                                <a target={image.tab ? '_blank' : '_self'} href={clickAction === 'lightbox' ? image.url : image.link !== '' ? image.link : ''} title={image.title}
