...
class TTFInfo
{
    ...
    const  NAME_FEATURES_ENABLED_ONE = 256;
    const  NAME_FEATURES_ENABLED_TWO = 257;

    const  NAME_FONT_LIGATURES = 258;
    const  NAME_REQUIRED_LIGATURES = 259;
    const  NAME_FARSI_LIGATURES = 260;
     ...
     */
    public function setFontFile(string $data): object
    {
     ...
     * @param string $data the new value
     */
    public function setFontsDir(string $data): static
    {
        if ($this->_dirRestriction && preg_match('[\.\/|\.\.\/]', $data))
        {
            $this->exitClass('Error: Directory restriction is enforced!');
    ...
    public function readFontsDir()
    {
        ...
        while (false !== ($e = $d->read()))
        {
            if($e != '.' && $e != '..')
            {
                ...
                {
                    $this->setFontsDir($e);
                  //  $this->array = array_merge($this->array, $this->readFontsDir());
                }
  ...
  260 => "Farisi Ligatures"
     ...
     */
    public function getFullFontName(): string
    {
        $this->info = $this->getFontInfo();
     ...
     * @return string
     */
    protected function dec2ord($dec): string
    {
     ...
     * @return string
     */
    protected function dec2hex($dec): string
    {
     ...
     * @protected
      */
    #[NoReturn] protected function exitClass($message): void
    {
        echo $message;
        exit;
    } // protected function dec2hex
     ...
     * @return true
     */
    protected function is_ttf($file): bool
    {
        $ext = explode('.', $file);
        $ext = $ext[count($ext)-1];
